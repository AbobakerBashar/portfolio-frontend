      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-base max-w-xl mx-auto" style={{ color: 'var(--muted-foreground)' }}>
            Technologies I use daily to build fast, scalable, and maintainable applications.
          </p>
        </motion.div>
          ...
          {filtered.map((skill, i) => {
            ...
            return (
              ...
              >
                {/* Icon */}
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center text-lg mb-4 font-mono font-bold text-sm"
                  style={{ background: `${color}15`, color: color }}
                >
                  {skill.icon}
                </div>

                <div className="font-semibold text-sm mb-3" style={{ color: 'var(--foreground)' }}>
                  {skill.name}
                </div>
          ...
          {Object.entries(SKILLS).map(([cat, skills]) => (
            <div key={cat} className="glass rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-sm" style={{ color: 'var(--foreground)' }}>{cat}</span>
              </div>
