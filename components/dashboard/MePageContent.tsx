...
export default function MePageContent({
	...
	const [settings, setSettings] = useState<SettingsInputs>(data || FORM_VALUES);
	const [avatar, setAvatar] = useState(data?.profile?.avatar?.url || "");
	const [resume, setResume] = useState({
		file: null as File | null,
		name: data?.resume?.url ? data.resume.url.split("/").pop() || "" : "",
	});
	...
	const loading =
		isUpdatingAvatar || isUpdatingResume || isCreating || isUpdating;
	...
	const handleUploadResume = async () => {
		...
		formData.append("publicId", data?.resume?.publicId || "");
		...
		if (res.success) {
			...
			if (res.errors) setErrors(res.errors);
				...
				setErrors({
					error: "Failed to update settings resume",
				});
		}
	};

	const resetProfile = () => {
		setSettings(data || FORM_VALUES);
		setSaved(true);
	};
		...
		<div className="space-y-8">
			...
			<div className="grid gap-8 xl:grid-cols-[1.4fr_0.9fr]">
				<form
					...
					style={{ border: "1px solid var(--border)" }}
				>
					{errors?.["profile.avatar.url"] && (
						<p className="text-destructive">{errors["profile.avatar.url"]}</p>
					)}
